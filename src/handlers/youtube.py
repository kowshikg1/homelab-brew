...
class Youtube(BaseAPI):
    ...
    def get_playlists(self, channel_id:str = None, mine:bool = False) -> list:
        ...
        while True:
            ...
            res = requests.get(f"{self.base_url}/playlists", headers=self.header, params=params, timeout=60)

            if res.status_code != 200:
                raise Exception(f"Failed to fetch playlists: {res.status_code} - {res.text}")
    ...
    def get_playlist_items(self, playlist_id: str) -> list:
        ...
        while True:
            ...
            res = requests.get(f"{self.base_url}/playlistItems", headers=self.header, params=params, timeout=60)

            if res.status_code != 200:
                raise Exception(f"Failed to fetch playlist items: {res.status_code} - {res.text}")
    ...
    def get_playlist_items_helper(self, playlist_ids: list[str]=[], channel_ids: list[str]=[], mine: bool = False) -> list[dict]:
        playlist_ids.extend(self.get_playlist_ids(channel_ids=channel_ids, mine=mine))
        all_items = []
