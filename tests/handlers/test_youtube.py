...
from src.handlers.youtube import Youtube, YOUTUBE_ENV_CONFIG
...
class TestGetPlaylists:
    def test_raises_when_mine_and_channel_id_both_given(self, yt):
        with pytest.raises(ValueError, match="Cannot use channel_id and mine together"):
            yt.get_playlists(channel_id="UC123", mine=True)

    def test_returns_single_page(self, yt):
        items = [{"id": "PL1"}, {"id": "PL2"}]
        with patch("src.handlers.youtube.requests.get", return_value=_make_page(items)):
    ...
    def test_paginates_across_multiple_pages(self, yt):
        ...
        with patch("src.handlers.youtube.requests.get", side_effect=[page1, page2]) as mock_get:
            result = yt.get_playlists(channel_id="UC123")
    ...
    def test_raises_on_non_200(self, yt):
        ...
        with patch("src.handlers.youtube.requests.get", return_value=mock_response):
    ...
    def test_mine_param_sent_in_request(self, yt):
        with patch("src.handlers.youtube.requests.get", return_value=_make_page([])) as mock_get:
...
class TestGetPlaylistIds:
    def test_returns_ids_for_mine(self, yt):
        with patch.object(yt, "get_playlists", return_value=[{"id": "PL_mine"}]):
    ...
    def test_deduplicates_ids(self, yt):
        with patch.object(yt, "get_playlists", return_value=[{"id": "PL_dup"}, {"id": "PL_dup"}]):
...
class TestGetPlaylistItems:
    def test_returns_items(self, yt):
        items = [{"snippet": {"title": "Video 1"}}, {"snippet": {"title": "Video 2"}}]
        with patch("src.handlers.youtube.requests.get", return_value=_make_page(items)):
    ...
    def test_raises_on_non_200(self, yt):
        ...
        with patch("src.handlers.youtube.requests.get", return_value=mock_response):
            with pytest.raises(Exception, match="Failed to fetch playlist items"):
                yt.get_playlist_items("PL_bad")

    def test_empty_playlist_returns_empty_list(self, yt):
        with patch("src.handlers.youtube.requests.get", return_value=_make_page([])):
...
class TestGetPlaylistItemsHelper:
    def test_combines_items_from_multiple_playlists(self, yt):
        ...
        with patch.object(yt, "get_playlist_ids", return_value=[]), \
    ...
    def test_extends_with_channel_playlist_ids(self, yt):
        with patch.object(yt, "get_playlist_ids", return_value=["PL_from_channel"]), \
             patch.object(yt, "get_playlist_items", return_value=[{"id": "item"}]) as mock_items:
            result = yt.get_playlist_items_helper(channel_ids=["UC1"])
