    ...
    def get_access_token_OAuth2(self, env_keys: APIEnvConfig) -> str:
        ...
        if expires_at and current_timestamp() >= float(expires_at):
            ...
            if res.status_code == 200:
                data = res.json()
                env_manager.set(
                    env_keys.ACCESS_TOKEN, data["access_token"],
                    **{
                        env_keys.REFRESH_TOKEN: data.get("refresh_token", refresh_token) or refresh_token,
                        env_keys.EXPIRES_AT: data.get("expires_at", str(current_timestamp() + int(data["expires_in"] - 60))),#60sec buffer
                    }
                )
