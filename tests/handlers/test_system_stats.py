...
def mock_psutil():
    ...
    with (
        patch(
            'src.handlers.system_stats.psutil.cpu_percent', return_value=25.0
        ) as mock_cpu,
        patch(
            'src.handlers.system_stats.psutil.getloadavg',
            return_value=(1.5, 1.2, 0.9),
        ) as mock_load,
        patch(
            'src.handlers.system_stats.psutil.virtual_memory', return_value=mem
        ),
        patch(
            'src.handlers.system_stats.psutil.swap_memory', return_value=swap
        ),
        patch(
            'src.handlers.system_stats.psutil.disk_usage', return_value=disk
        ) as mock_disk,
        patch(
            'src.handlers.system_stats.psutil.net_io_counters', return_value=net
        ),
        patch(
            'src.handlers.system_stats.psutil.boot_time',
            return_value=1_700_000_000.0,
        ),
        patch(
            'src.handlers.system_stats.psutil.sensors_temperatures',
            return_value={},
        ),
    ):
...
class TestGetCpuTemp:
    ...
    def test_cpu_temp_none_in_record_when_unavailable(self, handler):
        with (
            patch(
                'src.handlers.system_stats.psutil.cpu_percent',
                return_value=10.0,
            ),
            patch(
                'src.handlers.system_stats.psutil.getloadavg',
            ...
            patch(
                'src.handlers.system_stats.psutil.virtual_memory',
                return_value=MagicMock(total=1, used=1, percent=1.0),
            ),
            patch(
                'src.handlers.system_stats.psutil.swap_memory',
                return_value=MagicMock(total=0, used=0, percent=0.0),
            ),
            patch(
                'src.handlers.system_stats.psutil.disk_usage',
                return_value=MagicMock(total=1, used=1, percent=1.0),
            ),
            patch(
                'src.handlers.system_stats.psutil.net_io_counters',
