...
from src.handlers.strava import Strava, STRAVA_ENV_CONFIG
...
class TestGetActivities:
    def test_returns_empty_list_when_no_activities(self, strava_with_token):
        ...
        with patch("src.handlers.strava.requests.get", return_value=mock_response):
            result = strava_with_token.get_activities()

        assert result == []

    def test_returns_activities(self, strava_with_token):
        activity = {"id": 1, "name": "Morning Run", "start_date": "2026-01-01T07:00:00Z"}
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.side_effect = [[activity], []]  # page 1 has data, page 2 empty
    ...
    def test_paginates_until_empty_page(self, strava_with_token):
        ...
        with patch("src.handlers.strava.requests.get", return_value=mock_response) as mock_get:
            result = strava_with_token.get_activities()
...
class TestGetStreams:
    def test_returns_stream_for_each_activity(self, strava_with_token):
        stream_data = {"time": {"data": [0, 1, 2]}, "distance": {"data": [0.0, 5.0, 10.0]}}
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = stream_data

        with patch("src.handlers.strava.requests.get", return_value=mock_response):
            result = strava_with_token.get_streams([101, 102], keys=["time", "distance"])
...
class TestGetStreamsHelper:
    ...
    def test_calls_get_streams_with_activity_ids(self, strava_with_token):
        ...
        with patch.object(strava_with_token, "get_activities", return_value=activities), \
             patch.object(strava_with_token, "get_streams", return_value=streams) as mock_get_streams:
            result = strava_with_token.get_streams_helper()
    ...
    def test_attaches_start_date_to_streams(self, strava_with_token):
        ...
        with patch.object(strava_with_token, "get_activities", return_value=activities), \
             patch.object(strava_with_token, "get_streams", return_value=streams):
            result = strava_with_token.get_streams_helper()
    ...
    def test_custom_keys_passed_to_get_streams(self, strava_with_token):
        ...
        with patch.object(strava_with_token, "get_activities", return_value=activities), \
             patch.object(strava_with_token, "get_streams", return_value=streams) as mock_gs:
