...
from src.scripts.configs.compile_service_configs import (
    ServiceConfig,
    load_service_config,
)
...
def mock_env_for_service_config(tmp_path):
    ...
    with patch(
        'src.scripts.configs.compile_service_configs.EnvManager',
...
class TestServiceConfig:
    ...
    def test_accepts_exec_module(self):
        svc = ServiceConfig(
            name='svc', description='desc', exec_module='src.main'
        )
        assert svc.exec_module == 'src.main'

    def test_accepts_exec_file(self):
        svc = ServiceConfig(
            name='svc', description='desc', exec_file='/path/to/script.py'
        )
