...
class TestGetAccessTokenOAuth2:
    ...
    def test_refreshes_token_when_expired(self, env_config):
        ...
        mock_mgr.get.side_effect = lambda key, default=None: values.get(
            key, default
        )
        ...
        with (
            patch('src.handlers.base_api.EnvManager', return_value=mock_mgr),
            patch(
                'src.handlers.base_api.requests.post',
