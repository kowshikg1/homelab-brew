...
from src.utils.path_variables import PATH_INGESTION_CONFIG, ENV_FILE_GLOBAL

env = EnvManager(ENV_FILE_GLOBAL)

DEFAULT_PROJECT_ROOT = env.get("PROJECT_ROOT", Path(__file__).resolve().parents[3])
...
def _with_logging(command: str, job_name: str, job_config: dict[str, Any]) -> str:
...
def _with_lock(command: str, job_name: str, job_config: dict[str, Any]) -> str:
    ...
    if use_lock is None:
        use_lock = _is_frequent_schedule(str(job_config.get("schedule", "")).strip())
...
def _build_general_command(job_name: str, job_config: dict[str, Any]) -> str:
    ...
    if not command:
        ...
        if isinstance(commands, list) and commands:
            ...
            command = f" {operator} ".join(str(item).strip() for item in commands if str(item).strip())
    ...
    if not command:
        ...
        if script:
            ...
            run_with_python = bool(job_config.get("python", script.endswith(".py")))
            if run_with_python:
                python_bin = str(job_config.get("python_bin", ".venv/bin/python"))
...
def compile_crontab_jobs(
    ...
    for job_name, job_config in load_yaml(compiled_ingestion_file).items():
        ...
        job_config["append_log"] = logging.get("append_log", bool(job_config.get("log_file")))
    ...
    for config_file in _iter_yaml_files(Path(general_jobs_folder)):
        if config_file.resolve() == Path(output_file).resolve() or config_file.stem == 'example':
            continue
        ...
        for job_name, job_config in config_data.items():
            ...
            lines.append(f"{schedule} cd {job_config.get('repo') or normalized_project_root} && {command}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compile YAML cron jobs to crontab command lines")
    parser.add_argument("--ingestion-config", default=str(PATH_INGESTION_CONFIG))
    parser.add_argument("--general-jobs-folder", default=str(DEFAULT_CRONTAB_TASKS_FOLDER))
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT_FILE))
    parser.add_argument("--project-root", default=str(DEFAULT_PROJECT_ROOT))
    args = parser.parse_args()

    lines = compile_crontab_jobs(
        compiled_ingestion_file=Path(args.ingestion_config),
        general_jobs_folder=Path(args.general_jobs_folder),
