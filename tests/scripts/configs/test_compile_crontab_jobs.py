...
def mock_defaults(tmp_path, mock_env):
    ...
    with patch(
        'src.scripts.configs.compile_crontab_jobs.DEFAULT_PROJECT_ROOT',
        tmp_path,
    ):
        with patch(
            'src.scripts.configs.compile_crontab_jobs.DEFAULT_CRONTAB_TASKS_FOLDER',
            tmp_path / 'crontab',
        ):
            with patch(
                'src.scripts.configs.compile_crontab_jobs.DEFAULT_OUTPUT_FILE',
                tmp_path / 'compiled.crontab',
            ):
                with patch(
...
class TestBuildIngestionCommand:
    def test_basic_ingestion_command(self):
        ...
        assert (
            '.venv/bin/python -m src.ingestion.base_ingestion STRAVA_ACTIVITIES'
            in result
        )
    ...
    def test_ingestion_with_custom_python_bin(self):
        ...
        assert (
            '/usr/bin/python3 -m src.ingestion.base_ingestion YOUTUBE' in result
        )
...
class TestBuildGeneralCommand:
    ...
    def test_commands_list_with_and_operator(self):
        job_config = {
            'commands': ['echo start', 'python script.py', 'echo done'],
            'operator': '&&',
        }
    ...
    def test_commands_list_with_pipe_operator(self):
        job_config = {
            'commands': ['cat file.txt', 'grep pattern'],
            'operator': '|',
        }
    ...
    def test_script_with_args(self):
        job_config = {
            'script': 'src/scripts/backup.py',
            'args': ['--dir=/data', '--compress'],
        }
...
class TestCompileCrontabJobs:
    ...
    def test_logging_config_from_nested_structure(
    ...
    def test_frequent_ingestion_job_uses_lock_by_default(
        self, tmp_path, mock_defaults
    ):
        ingestion_config = tmp_path / 'ingestion_config.json'
        _write_json(
    ...
    def test_infrequent_ingestion_job_no_lock_by_default(
        self, tmp_path, mock_defaults
    ):
        ingestion_config = tmp_path / 'ingestion_config.json'
        _write_json(
